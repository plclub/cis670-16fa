...
Require Export systemt_finite_ott.
...
Fixpoint close_exp_wrt_exp_rec (n1 : nat) (x1 : tmvar) (e1 : exp) {struct e1} : exp :=
  match e1 with
    ...
    | fapp e2 e3 => fapp (close_exp_wrt_exp_rec n1 x1 e2) (close_exp_wrt_exp_rec n1 x1 e3)
    | null => null
    | pair e2 e3 => pair (close_exp_wrt_exp_rec n1 x1 e2) (close_exp_wrt_exp_rec n1 x1 e3)
    | fst e2 => fst (close_exp_wrt_exp_rec n1 x1 e2)
    | snd e2 => snd (close_exp_wrt_exp_rec n1 x1 e2)
    | abort t1 e2 => abort t1 (close_exp_wrt_exp_rec n1 x1 e2)
    | inl t1 e2 => inl t1 (close_exp_wrt_exp_rec n1 x1 e2)
    | inr t1 e2 => inr t1 (close_exp_wrt_exp_rec n1 x1 e2)
    | scase e2 e3 e4 => scase (close_exp_wrt_exp_rec n1 x1 e2) (close_exp_wrt_exp_rec (S n1) x1 e3) (close_exp_wrt_exp_rec (S n1) x1 e4)
  end.
...
Fixpoint size_typ (t1 : typ) {struct t1} : nat :=
  match t1 with
    ...
    | typ_unit => 1
    | typ_prod t2 t3 => 1 + (size_typ t2) + (size_typ t3)
    | typ_void => 1
...
Fixpoint size_exp (e1 : exp) {struct e1} : nat :=
  match e1 with
    ...
    | fapp e2 e3 => 1 + (size_exp e2) + (size_exp e3)
    | null => 1
    | pair e2 e3 => 1 + (size_exp e2) + (size_exp e3)
    | fst e2 => 1 + (size_exp e2)
    | snd e2 => 1 + (size_exp e2)
    | abort t1 e2 => 1 + (size_typ t1) + (size_exp e2)
    | inl t1 e2 => 1 + (size_typ t1) + (size_exp e2)
    | inr t1 e2 => 1 + (size_typ t1) + (size_exp e2)
    | scase e2 e3 e4 => 1 + (size_exp e2) + (size_exp e3) + (size_exp e4)
  end.
...
Inductive degree_exp_wrt_exp : nat -> exp -> Prop :=
  ...
  | degree_wrt_exp_fapp : forall n1 e1 e2,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 e2 ->
    degree_exp_wrt_exp n1 (fapp e1 e2)
  | degree_wrt_exp_null : forall n1,
    degree_exp_wrt_exp n1 (null)
  | degree_wrt_exp_pair : forall n1 e1 e2,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 e2 ->
    degree_exp_wrt_exp n1 (pair e1 e2)
  | degree_wrt_exp_fst : forall n1 e1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (fst e1)
  | degree_wrt_exp_snd : forall n1 e1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (snd e1)
  | degree_wrt_exp_abort : forall n1 t1 e1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (abort t1 e1)
  | degree_wrt_exp_inl : forall n1 t1 e1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (inl t1 e1)
  | degree_wrt_exp_inr : forall n1 t1 e1,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp n1 (inr t1 e1)
  | degree_wrt_exp_scase : forall n1 e1 e2 e3,
    degree_exp_wrt_exp n1 e1 ->
    degree_exp_wrt_exp (S n1) e2 ->
    degree_exp_wrt_exp (S n1) e3 ->
    degree_exp_wrt_exp n1 (scase e1 e2 e3).
...
Inductive lc_set_exp : exp -> Set :=
  ...
  | lc_set_fapp : forall e1 e2,
    lc_set_exp e1 ->
    lc_set_exp e2 ->
    lc_set_exp (fapp e1 e2)
  | lc_set_null :
    lc_set_exp (null)
  | lc_set_pair : forall e1 e2,
    lc_set_exp e1 ->
    lc_set_exp e2 ->
    lc_set_exp (pair e1 e2)
  | lc_set_fst : forall e1,
    lc_set_exp e1 ->
    lc_set_exp (fst e1)
  | lc_set_snd : forall e1,
    lc_set_exp e1 ->
    lc_set_exp (snd e1)
  | lc_set_abort : forall t1 e1,
    lc_set_exp e1 ->
    lc_set_exp (abort t1 e1)
  | lc_set_inl : forall t1 e1,
    lc_set_exp e1 ->
    lc_set_exp (inl t1 e1)
  | lc_set_inr : forall t1 e1,
    lc_set_exp e1 ->
    lc_set_exp (inr t1 e1)
  | lc_set_scase : forall e1 e2 e3,
    lc_set_exp e1 ->
    (forall x1 : tmvar, lc_set_exp (open_exp_wrt_exp e2 (var_f x1))) ->
    (forall x2 : tmvar, lc_set_exp (open_exp_wrt_exp e3 (var_f x2))) ->
    lc_set_exp (scase e1 e2 e3).
...
Lemma lc_scase_exists :
forall x1 x2 e1 e2 e3,
  lc_exp e1 ->
  lc_exp (open_exp_wrt_exp e2 (var_f x1)) ->
  lc_exp (open_exp_wrt_exp e3 (var_f x2)) ->
  lc_exp (scase e1 e2 e3).
Proof.
intros; exp_lc_exists_tac; eauto with lngen.
Qed.
...
Hint Extern 1 (lc_exp (scase _ _ _)) =>
  let x1 := fresh in
  pick_fresh x1;
  let x2 := fresh in
  pick_fresh x2;
  apply (lc_scase_exists x1 x2).
...
Hint Resolve lc_body_scase_2 : lngen.

Lemma lc_body_scase_3 :
forall e1 e2 e3,
  lc_exp (scase e1 e2 e3) ->
  body_exp_wrt_exp e3.
Proof.
...
Hint Resolve lc_body_scase_3 : lngen.
...
Lemma subst_exp_scase :
forall x2 x3 e2 e3 e4 e1 x1,
  lc_exp e1 ->
  x2 `notin` fv_exp e1 `union` fv_exp e3 `union` singleton x1 ->
  x3 `notin` fv_exp e1 `union` fv_exp e4 `union` singleton x1 ->
  subst_exp e1 x1 (scase e2 e3 e4) = scase (subst_exp e1 x1 e2) (close_exp_wrt_exp x2 (subst_exp e1 x1 (open_exp_wrt_exp e3 (var_f x2)))) (close_exp_wrt_exp x3 (subst_exp e1 x1 (open_exp_wrt_exp e4 (var_f x3)))).
Proof.
default_simp.
Qed.

Hint Resolve subst_exp_scase : lngen.
