...
Lemma eq_exp_dec : forall e e' : exp,
    { e = e' } + { e <> e' }.
Proof.
  Admitted.
...
Module CallByNeed.
  ...
  Inductive typing : env -> sig_table -> exp -> typ -> nat -> Prop :=
  | typing_sym : forall E (a : atom) T Σ,
      uniq E
      -> uniq Σ
      -> binds a T Σ
      -> typing E Σ (exp_sym a) T O
  | typing_var : forall E (x : atom) T Σ,
      uniq E
      -> binds x T E
      -> typing E Σ (exp_fvar x) T O
  | typing_zero : forall E Σ,
      uniq E
      -> typing E Σ exp_zero typ_nat O
  | typing_succ : forall E e Σ k,
      typing E Σ e typ_nat k
      -> typing E Σ (exp_succ e) typ_nat (S k)
  | typing_ifz : forall (L : atoms) E num z_branch s_branch t Σ k1 k2 k3,
      typing E Σ num typ_nat k1
      -> typing E Σ z_branch t k2
      -> (forall (x:atom), x `notin` L
                     -> typing ((x ~ typ_nat) ++ E) Σ (open s_branch (exp_fvar x)) t k3)
      -> typing E Σ (exp_ifz num z_branch s_branch) t (S (max (max k1 k2) k3))
  | typing_abs : forall (L : atoms) E exp t1 t2 Σ k,
      (forall (x:atom), x `notin` L
                   -> typing ((x ~ t1) ++ E) Σ (open exp (exp_fvar x)) t2 k)
      -> typing E Σ (exp_abs t1 exp) (typ_parr t1 t2) (S k)
  | typing_app : forall E e1 e2 t1 t2 Σ k1 k2,
      typing E Σ e1 (typ_parr t1 t2) k1
      -> typing E Σ e2 t1 k2
      -> typing E Σ (exp_app e1 e2) t2 (S (max k1 k2))
  | typing_fix : forall (L : atoms) E exp t Σ k,
      (forall (x:atom), x `notin` L
                   -> typing ((x ~ t) ++ E) Σ (open exp (exp_fvar x)) t k)
      -> typing E Σ (exp_fix t exp) t (S k).
  
  Inductive typing_mu : sig_table -> mu_table -> sig_table -> nat -> Prop :=
  | typing_mu_nil  : forall Σ', uniq Σ' -> typing_mu Σ' nil nil O
  | typing_mu_cons_present : forall Σ' Σ mu e t a k_mu k_e,
      typing_mu Σ' mu Σ k_mu ->
      typing nil Σ' e t k_e ->
      e <> exp_blackhole ->
      a `notin` dom Σ' ->
      a `notin` dom Σ ->
      a `notin` dom mu ->
      typing_mu ((a ~ t) ++ Σ') ((a ~ e) ++ mu) ((a ~ t) ++ Σ) (max k_mu k_e)
  | typing_mu_cons_blackhole : forall Σ' Σ mu t a k_mu,
      typing_mu Σ' mu Σ k_mu ->
      a `notin` dom Σ' ->
      a `notin` dom Σ ->
      a `notin` dom mu ->
      typing_mu ((a ~ t) ++ Σ') ((a ~ exp_blackhole) ++ mu) ((a ~ t) ++ Σ) k_mu.
  
                (*
  | typing_mu_junk : forall Σ mu e a,
      typing_mu Σ' mu Σ ->
      a `notin` dom Σ ->
      a `notin` dom mu ->
  
    typing_mu Σ' ((a,e)::mu) Σ
                 *)
  ...
  Lemma typing_weakening_sigma : forall E Σ a e t t2 k,
      typing E Σ e t k ->
      uniq ((a ~ t2) ++ Σ) ->
      typing E ((a ~ t2) ++ Σ) e t k.
  Proof.
  Admitted.

  Lemma typing_weakening_sigma_strengthened : forall E Σ Σ' e t k,
      Σ ⊆ Σ' ->
      uniq Σ' ->
      typing E Σ e t k->
      typing E Σ' e t k.
  ...
  Lemma typing_mu_subst : forall a Σ' Σ e e' k_e t mu k_mu,
      typing nil Σ' e t k_e ->
      typing_mu Σ' ((a ~ e') ++ mu) Σ k_mu ->
      exists k_mu', typing_mu Σ' ((a ~ e) ++ mu) Σ k_mu'.
  Proof.
    intros a Σ' Σ e e' k_e t mu k_mu Htyping_e Htyping_mu.
    generalize dependent Htyping_e.
    generalize dependent e.
    generalize dependent t.
    generalize dependent k_e.
    remember ((a ~ e') ++ mu) as mu_.
    generalize dependent mu.
    generalize dependent e'.
    generalize dependent a.
    induction Htyping_mu; intros; inversion Heqmu_; subst; clear Heqmu_.
    Admitted.
    
  Lemma typing_mu_safe_inversion : forall Σ' Σ mu k_mu,
      typing_mu Σ' mu Σ k_mu ->
      uniq Σ' /\ uniq Σ /\ uniq mu /\
          (forall a t, binds a t Σ
                  -> exists e k_e, binds a e mu
                         /\ (e <> exp_blackhole
                            -> (typing nil Σ' e t k_e /\ k_e <= k_mu))).
  Proof.
    intros Σ' Σ mu k_mu Htyping_mu.
    induction Htyping_mu.
    - split; auto.
      split; auto.
      split; auto.
      intros a t Hbinds.
        ...
        * split.
          ...
          intros a0 t0 Hbinds.
          {
            ...
            - subst.
              ...
              intros Hneq.
              split; auto.
              apply typing_weakening_sigma; try solve_uniq.
              apply Max.le_max_r.              
            - destruct IHHtyping_mu as [Huniq_sigma' [Huniq_sigma [Huniq_mu IHHtyping_mu']]].
              assert (binds a0 t0 Σ). {
                rewrite_env (nil ++ (a ~ t) ++ Σ) in Hbinds.
                rewrite_env (nil ++ Σ).
                eapply binds_remove_mid; eauto.
              }
              destruct (IHHtyping_mu' a0 t0 H4) as [e' [k_e' [Hbinds_e' Htyping_e']]].
              exists e', k_e'.
              split; auto.
              split; auto.
              + apply Htyping_e' in H5.
                destruct H5 as [Htyping_e'2 k_e'_le_k_mu].
                apply typing_weakening_sigma; try solve_uniq.
              + apply Htyping_e' in H5.
                destruct H5 as [Htyping_e'2 k_e'_le_k_mu].
                assert (k_mu <= max k_mu k_e). {
                  apply Max.le_max_l.
                }
                eapply Nat.le_trans; eauto.
          }
    - split; try solve_uniq.
      split; try solve_uniq.
      split; try solve_uniq.
      intros a0 t0 Hbinds.
      destruct (a0 == a).
        + subst.
          exists exp_blackhole, 0.
          split.
          * auto.
          * intros contra.
            solve_neq_contra.
        + destruct IHHtyping_mu as [Huniq_sigma' [Huniq_sigma [Huniq_mu IHHtyping_mu']]].
          ...
          destruct (IHHtyping_mu' a0 t0 H2) as [e' [k_e' [Hbinds_e' Htyping_e']]].
          exists e', k_e'.
          split; auto.
          split; auto.
          apply typing_weakening_sigma; try solve_uniq.
          apply Htyping_e' in H3.
          destruct H3; auto.
  Qed.

  Lemma typing_not_blackhole : forall E Σ e T k,
      typing E Σ e T k -> e <> exp_blackhole.
  Proof.
    intros E Σ e T k Htyping contra.
  ...
  | eval_state_safe : forall Σ e t mu k_e k_mu, typing nil Σ e t k_e
                                           -> typing_mu Σ mu Σ k_mu
  ...
  | cbn_smallstep_inplace :
      forall e e' Σ Σ' a T mu mu' k,
        lc_by_need e mu
        -> uniq ((a ~ exp_blackhole) ++ mu)
        -> uniq Σ
        -> binds a T Σ
        -> binds a T Σ'
        -> cbn_smallstep (Σ, e, (a ~ exp_blackhole) ++ mu)
                        (Σ', e', (a ~ exp_blackhole) ++ mu') k
        -> e <> exp_blackhole
        -> e' <> exp_blackhole
        -> cbn_smallstep (Σ, (exp_sym a), (a ~ e) ++ mu)
                        (Σ', (exp_sym a), (a ~ e') ++ mu') k
  | cbn_smallstep_succ :
  ...
  Lemma typing_subst : forall Σ E e x u T1 T2 k_e k_u,
      typing ((x ~ T1) ++ E) Σ e T2 k_e ->
      typing E Σ u T1 k_u->
      exists k_subst, typing E Σ ([x ~> u] e) T2 k_subst.
  Proof.
    Admitted.
  ...
  Proof.
    ...
    - constructor.
      ...
      + subst.
        ...
        auto.
      + apply H10 with (x := x).
        rewrite_env (nil ++ (a ~ e0) ++ mu0) in H8.
        rewrite_env (nil ++ mu0).
        eapply binds_remove_mid; eauto.
  Qed.
  ...
  Proof.
    ...
    - intros e' e0 mu' Heqtarget_st mu0 Heqstart_st. 
      ...
      inversion H6; subst.
      eapply H8; eauto.
    - intros e1 e0 mu'0 Heqtarget_st mu0 Heqstart_st.
      inversion Heqtarget_st; inversion Heqstart_st; subst.
      inversion H; subst.
      split; auto.
      + constructor.
      + edestruct IHcbn_smallstep with (mu'0 := (a ~ exp_blackhole) ++ mu')
                                       (mu0  := (a ~ exp_blackhole) ++ mu).
        reflexivity.
        reflexivity.
        inversion H10; subst.
        constructor.
        ...
        * intros.
          {
            ...
            - subst.
              ...
              auto.
            - apply H12 with (x := x).
              rewrite_env (nil ++ (a ~ e') ++ mu') in H13.
              rewrite_env (nil ++ (a ~ exp_blackhole) ++ mu').
  ...
  Lemma preservation_strengthened : forall Σ Σ' e e' mu mu' t k k_ty_mu k_ty_e,
      cbn_smallstep (Σ, e, mu) (Σ', e', mu') k
      -> typing_mu Σ mu Σ k_ty_mu
      -> typing nil Σ e t k_ty_e
      -> exists k_ty_mu' k_ty_e',
          typing_mu Σ' mu' Σ' k_ty_mu' /\ typing nil Σ' e' t k_ty_e' /\ Σ ⊆ Σ'.
  Proof.
    intros Σ Σ' e e' mu mu' t k k_ty_mu k_ty_e Hstep Htyping_mu Htyping_e.
    remember (Σ, e, mu) as start_st.
    remember (Σ', e', mu') as target_st.
    generalize dependent Σ.
    generalize dependent Σ'.
    ...
    generalize dependent e'.
    generalize dependent t.
    generalize dependent k_ty_mu.
    generalize dependent k_ty_e.
    induction Hstep.
    - Case "Lookup".
      intros k_ty_e k_ty_mu t e1 e0 mu1 m0 Σ1 Heqtarget_st Σ0 Heqstart_st.
      inversion Heqtarget_st; inversion Heqstart_st; subst.
      intros Htyping_mu0 Htyping_a.
      subst.
      edestruct typing_mu_safe_inversion as [Huniq1 [Huniq2 [Huniq_mu Htyping]]]; eauto.
      destruct (Htyping a T H1) as [e' [k_e' [Hbinds Htyping_e']]].
      assert (e1 = e'). {
        eapply binds_unique; eauto.
      }
      subst.
    ...
    - Case "InPlace".
      ...
      + edestruct IHHstep as [k_ty_mu' [k_ty_e' [Htyping_mu'0 [Htypinge'0 Hsubset]]]]; eauto.
        apply typing_mu_cons_blackhole; auto.
        apply H14; auto.
        apply typing_weakening_sigma; auto.
        apply H15; auto.
        exists k_ty_mu', k_ty_e'.
        split; auto.
        eapply typing_mu_subst.
  ...
  (* Mutual induction principle for expression and mu table *)
  Lemma typing_mu_mutual_ind :
    forall (P : env -> sig_table -> exp -> typ -> Prop)
      (P0 : sig_table -> mu_table -> sig_table -> Prop),
       (forall (E : env) (a : atom) (T : typ) (Σ : env),
        uniq E -> uniq Σ -> binds a T Σ -> P E Σ (exp_sym a) T) ->
       (forall (E : env) (x : atom) (T : typ) (Σ : sig_table),
        uniq E -> binds x T E -> P E Σ (exp_fvar x) T) ->
       (forall (E : env) (Σ : sig_table), uniq E -> P E Σ exp_zero typ_nat) ->
       (forall (E : env) (e : exp) (Σ : sig_table),
        typing E Σ e typ_nat -> P E Σ e typ_nat -> P E Σ (exp_succ e) typ_nat) ->
       (forall (L : atoms) (E : env) (num z_branch s_branch : exp) (t : typ) (Σ : sig_table),
        typing E Σ num typ_nat ->
        P E Σ num typ_nat ->
        typing E Σ z_branch t ->
        P E Σ z_branch t ->
        (forall x : atom,
         x `notin` L -> typing ((x ~ typ_nat) ++ E) Σ (open s_branch (exp_fvar x)) t) ->
        (forall x : atom,
         x `notin` L -> P ((x ~ typ_nat) ++ E) Σ (open s_branch (exp_fvar x)) t) ->
        P E Σ (exp_ifz num z_branch s_branch) t) ->
       (forall (L : atoms) (E : env) (exp : exp) (t1 t2 : typ) (Σ : sig_table),
        (forall x : atom, x `notin` L -> typing ((x ~ t1) ++ E) Σ (open exp (exp_fvar x)) t2) ->
        (forall x : atom, x `notin` L -> P ((x ~ t1) ++ E) Σ (open exp (exp_fvar x)) t2) ->
        P E Σ (exp_abs t1 exp) (typ_parr t1 t2)) ->
       (forall (E : env) (e1 e2 : exp) (t1 t2 : typ) (Σ : sig_table),
        typing E Σ e1 (typ_parr t1 t2) ->
        P E Σ e1 (typ_parr t1 t2) ->
        typing E Σ e2 t1 -> P E Σ e2 t1 -> P E Σ (exp_app e1 e2) t2) ->
       (forall (L : atoms) (E : env) (exp : exp) (t : typ) (Σ : sig_table),
        (forall x : atom, x `notin` L -> typing ((x ~ t) ++ E) Σ (open exp (exp_fvar x)) t) ->
        (forall x : atom, x `notin` L -> P ((x ~ t) ++ E) Σ (open exp (exp_fvar x)) t) ->
        P E Σ (exp_fix t exp) t) ->
       (forall (Σ' Σ : env) (mu : list (atom * exp)),
        uniq Σ' ->
        uniq Σ ->
        uniq mu ->
        (forall (a : atom) (t : typ),
         binds a t Σ ->
         exists e : exp, binds a e mu /\ (e <> exp_blackhole -> typing nil Σ' e t)) ->
        P0 Σ' mu Σ) ->
       (forall (E : env) (Σ : sig_table) (e : exp) (t : typ), typing E Σ e t -> P E Σ e t)
       /\
       (forall (Σ' Σ : sig_table) (mu : mu_table), typing_mu Σ' mu Σ -> P0 Σ' mu Σ).
  Proof.
    intros.
    split.
    apply (typing_ind_2 P P0); assumption.
    intros S S' mu typing_mu.
    apply (typing_mu_ind_2 P0); assumption.
  Qed.

  Definition progress_exp (E : env) (Σ : sig_table) (e : exp) (t : typ) :=
      forall mu, (cbn_eval_state_final (Σ, e, mu)
             \/ cbn_loop (Σ, e, mu)
